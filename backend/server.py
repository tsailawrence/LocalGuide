...
def compute_user_embedding():
    ...
    set_user_embedding(user_id, vector)
...
def predict():
    ...
    if user_embedding != "default":
        ...
        for id, embedding in rest_embeddings:
            score = service.RecommendRestaurant(user_embedding, embedding)
            result.append((id, score))
        return {"message": "success", "content": "predict success", "result": result[:3]}
    else:
